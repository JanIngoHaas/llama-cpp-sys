...
//         let mut used_length = llama_tokenize(model, prompt_str.as_ptr(), prompt_str.as_bytes().len() as i32, tokens_list.as_mut_ptr(), tokens_list.len() as i32, true, true) as usize;
...
//             let used_tokens = &tokens_list[0..used_length];
//             let result_code = llama_decode(ctx, used_tokens.as_ptr(), used_length as i32, existing_token_count, 8);
//             if result_code == 1 {
//                 println!("eval failed");
//                 exit(1);
//             }
